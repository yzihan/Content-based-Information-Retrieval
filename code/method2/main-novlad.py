#!/usr/bin/env python
import os
from novlad import SiftDescriptorPrediction
...
for pos in range(K_FOLD):
    ...
    sift_descriptor_class = SiftDescriptorPrediction()

    ta = time.time()

    sift_descriptors, tests = sift_descriptor_class.get_clusters_descriptors(dataset, test_split = test_split, test_pos = test_pos)

    print('Setting up accelerator...')
    tb = time.time()

    tree = BallTree([ sift_descriptors[i]['feature_vector'] for i in range(len(sift_descriptors)) ])

    print('Describing...')
    a = time.time()

    prepared = sift_descriptor_class.prepare_query(tests.get_data())

    print('Predicting...')
    b = time.time()

    res_top = sift_descriptor_class.get_prediction(sift_descriptors, DISTANCE, prepared, mode=2, accelerator=tree, nearby_count=KNN_K)
    ...
    for pred_result, correct in zip(res_top, list(tests.get_data()['class_image'])):
        res_count = Counter([ sift_descriptors[x[0]]['class_image'] for x in pred_result ])

        class_pred, pred_count = min(res_count.items(), key=lambda x: (-x[1], x[0]))
        if pred_count == 1: # not reliable result
            class_pred = sift_descriptors[pred_result[0][0]]['class_image']
