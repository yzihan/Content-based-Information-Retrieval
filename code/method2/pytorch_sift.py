...
class SIFTNet(nn.Module):
    ...
    def forward(self, x):
        gx = self.gx(F.pad(x, (1, 1, 0, 0), 'replicate'))   # gradient at x axis (of each pixel)
        gy = self.gy(F.pad(x, (0, 0, 1, 1), 'replicate'))   # gradient at y axis (of each pixel)
        mag = torch.sqrt(gx * gx + gy * gy + self.eps)      # magnitude (of each pixel)
        ori = torch.atan2(gy, gx + self.eps)                # orientation (of each pixel)
